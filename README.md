# 📌 Front-End 개발자 채용 과제 – bixpayments

## 1. 프로젝트 개요

본 프로젝트는 **프론트엔드 개발자 채용 과제**로,  
제공된 API 서버와 연동하여 **회원 인증 및 게시판 기능을 구현한 SPA**입니다.

React 기반으로 구현되었으며,  
실제 서비스 환경을 고려해 **인증 처리, 라우팅 보호, 상태 관리, 코드 구조화**에 중점을 두었습니다.
...
- **비로그인 상태**
  - `/login`, `/signup` 페이지만 접근 가능
- **로그인 상태**
  - 게시판 관련 페이지 접근 가능
- 인증되지 않은 접근 시 로그인 페이지로 자동 리다이렉트
...
- 401 응답 발생 시 Refresh Token을 통한 재발급 처리
...
과제 수행 과정에서 실제 서비스 개발을 가정하고 구현하였습니다.
